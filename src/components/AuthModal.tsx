...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'profiles', userCred.user.uid), profile, { merge: true });
      onLoginSuccess(profile);
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-stone-50 rounded-3xl max-w-sm w-full overflow-hidden border border-stone-200 shadow-2xl flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-stone-200/80 flex items-center justify-between bg-stone-100/60">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-full hover:bg-stone-200 text-stone-400 hover:text-stone-700 cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 rounded-xl bg-stone-900 text-white text-xs font-semibold hover:bg-stone-800 shadow-sm flex items-center justify-center gap-1.5 cursor-pointer"
            >
              {loading ? (
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-2.5 rounded-xl bg-amber-50 text-amber-900 border border-amber-200/80 text-xs font-semibold hover:bg-amber-100 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
          >
